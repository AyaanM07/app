...
export const useFormsStore = create((set, get) => ({
  ...
  fetchFormsByClass: async (className, folderId) => {
    if (!folderId) return;
    
    try {
      set({ isLoading: true, error: null });
      
      console.log(`Fetching forms for class ${className} with folder ID ${folderId}`);
      
      // Use the new endpoint
      const response = await axios.get(`/api/questions/forms-extract?folderId=${folderId}`);
